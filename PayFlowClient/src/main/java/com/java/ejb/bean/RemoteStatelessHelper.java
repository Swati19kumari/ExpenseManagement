...
public class RemoteStatelessHelper {
    ...
    public static GroupBeanRemote lookupRemoteStatelessGroup() throws NamingException {
        Context ctx = createInitialContext();
        ...
        final String viewClassName = GroupBeanRemote.class.getName(); // remote interface

        return (GroupBeanRemote) ctx.lookup(
            "ejb:" + appName + "/" + moduleName + "/" + distinctName + "/" + beanName + "!" + viewClassName
        );
    }
}
